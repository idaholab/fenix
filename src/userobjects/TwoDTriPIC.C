...
registerMooseObject("FenixApp", TwoDTriPIC);

InputParameters
TwoDTriPIC::validParams()
{
...
TwoDTriPIC::TwoDTriPIC(const InputParameters & parameters)
...
void
TwoDTriPIC::initializeParticles()
{
  ...
  for (auto elem : *_fe_problem.mesh().getActiveLocalElementRange())
  {
    for (int i = 0; i < _particles_per_element; ++i)
    {
      ...
      if (data[1] > 1 - data[0])
      {
        Real distance = std::abs(- data[0] - data[1] + 1) / std::sqrt(2);
        data[0] = data[0] - 2 * distance / std::sqrt(2);
        data[1] = data[1] - 2 * distance / std::sqrt(2);
      }
